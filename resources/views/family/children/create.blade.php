...
<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Ajouter un enfant</h2>
            <div class="mt-1 text-sm text-gray-600">Créer un profil enfant (sans compte de connexion).</div>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('family.children.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input id="last_name" name="last_name" type="text" value="{{ old('last_name') }}" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" />
                                @error('last_name')
                                    <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <input id="birth_date" name="birth_date" type="date" value="{{ old('birth_date') }}" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" />
                            @error('birth_date')
                                <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="rounded-md border border-gray-200 bg-gray-50 p-4">
                            ...
                            <div class="mt-1 text-xs text-gray-600">Par défaut, vous êtes ajouté(e) comme tuteur (édition activée).</div>

                            <div class="mt-3 space-y-2">
                                @foreach($users as $u)
                                    @php
                                        $enabled = old('guardians.' . $u->id . '.enabled', $u->id === $meId ? 1 : 0);
                                        $canEdit = old('guardians.' . $u->id . '.can_edit', $u->id === $meId ? 1 : 0);
                                        $notify = old('guardians.' . $u->id . '.notify', $u->id === $meId ? 1 : 0);
                                    @endphp
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ route('family.index') }}" class="text-sm text-gray-600 hover:text-gray-900">Annuler</a>
                        </div>
                    </form>
                </div>
            </div>
