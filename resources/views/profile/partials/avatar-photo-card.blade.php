...
@if($embedded)
    <div id="profile-avatar" data-upload-url="{{ url('/profile/avatar') }}">
        <button type="button" data-action="change" class="block h-10 w-10 rounded-full overflow-hidden bg-[color:var(--fam-surface-2)] border border-[color:var(--fam-border-soft)]" aria-label="Changer la photo de profil">
            @if($avatarUrl !== '')
                <img src="{{ $avatarUrl }}" alt="" class="h-full w-full object-cover" loading="lazy" />
            @else
                <div class="h-full w-full flex items-center justify-center text-xs font-extrabold text-[color:var(--fam-muted)]">
                    {{ $user->initials() }}
                </div>
            @endif
        </button>
...
<div class="rounded-2xl border border-[color:var(--fam-border)] bg-white p-5" id="profile-avatar" data-upload-url="{{ url('/profile/avatar') }}">
